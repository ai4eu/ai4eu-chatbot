...
@register('ai4eu_transformers_mpnet_embedder')
class AI4EUTorchTransformersMPNetEmbedder(Serializable):
    """Transformers-based MPNET model for embeddings tokens, subtokens and sentences

    Args:
        load_path: path to a pretrained MPNet pytorch checkpoint
        config_file: path to a MPNet configuration file
        truncate: whether to remove zero-paddings from returned data

    """
    model: transformers.MPNetModel
    dim: int

    def __init__(self, load_path: Union[str, Path], mpnet_config_path: Union[str, Path] = None,
                 truncate: bool = False, **kwargs):
        super().__init__(save_path=None, load_path=load_path, **kwargs)
        if mpnet_config_path is not None:
            mpnet_config_path = expand_path(mpnet_config_path)
        self.config = mpnet_config_path
    ...
    def load(self):
        self.model = transformers.MPNetModel.from_pretrained(self.load_path, config=self.config).eval().to(self.device)
    ...
    def __call__(self,
        ...
        with torch.no_grad():
            ...
            # PP my code for mpnet
            token_embeddings = last_hidden  # First element of model_output contains all token embeddings
            input_mask_expanded = attention_tensor.expand(token_embeddings.size()).float()
            mean_emb = torch.sum(token_embeddings * input_mask_expanded, 1) / torch.clamp(input_mask_expanded.sum(1), min=1e-9)
